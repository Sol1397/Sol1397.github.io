...
// URL da API
const apiUrl = 'https://api.astronomyapi.com/v2/bodies';
...
fetch(`${apiUrl}?start_date=${startDate}&end_date=${endDate}`, {
  ...
  .then(data => {
    // Processar os dados dos planetas
    const planets = data.results;
    let planetHtml = '';

    planets.forEach(planet => {
      const sign = planet.sign ? planet.sign : 'Desconhecido';

      planetHtml += `<p>${planet.name}: ${sign}</p>`;
    });

    // Exibir os dados dos planetas no elemento HTML
